import { PencilCircle, PencilLine, PlusCircle } from 'phosphor-react';
    ...
    focus:ring-green-500
     uppercase  font-bold'
              ...
              <section className='mt-4 h-80'>
                <div className='flex'>
                  <span className='md:w-4/12 w-9/12 text-xs text-gray-700 font-bold '>
                    Modalidade
                    <p className='block md:hidden text-xs text-gray-700 font-bold '>
                      Responsável
                    </p>
                  </span>
                  <span className='w-4/12 hidden md:block text-xs text-gray-700 font-bold '>
                    Responsável
                  </span>
                  <span className='w-2/12 text-xs text-gray-700 font-bold '>
                    Telefone
                  </span>
                  <span className='w-1/12 hidden md:block text-xs text-gray-700 font-bold '>
                    Naipe
                  </span>
                  <span className='w-1/12 text-xs text-gray-700 font-bold '></span>
                </div>
                <ul className='h-full'>
                  {modalidadesData.map(({ answerable, naipe, name, phone }) => (
                    <li key={phone}>
                      <div className='flex  text-gray-600 dark:text-gray-200 justify-between py-3 border-b-2 border-gray-100 dark:border-gray-800'>
                        <span className='md:w-4/12 w-7/12 text-xs text-gray-700  '>
                          {name}
                          <p className='block md:hidden text-xs text-gray-700  '>
                            {answerable}
                          </p>
                        </span>
                        <span className='w-4/12 hidden md:block text-xs text-gray-700  '>
                          {answerable}
                        </span>
                        <span className='md:w-2/12 w-4/12 text-xs  text-gray-700  '>
                          {phone}
                        </span>
                        <span className='w-1/12 hidden md:block text-xs text-gray-700  '>
                          {naipe}
                        </span>
                        <button className='flex md:items-center justify-end w-1/12 mx-auto text-xs text-gray-700  '>
                          <PencilLine />
                        </button>
                      </div>
                    </li>
